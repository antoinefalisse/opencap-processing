...
def plotResultsOpenSimAD(dataDir, motion_filename, settings={},
    ...
    if not grfPlotOnly:
        ...
        MCF_labels = optimaltrajectories[cases[0]]['MCF_labels']
        nMCFs = len(MCF_labels)
        ny = np.ceil(np.sqrt(nMCFs))
        fig, axs = plt.subplots(1, int(ny))
        fig.suptitle('MCFs', fontsize=fontsizeSubTitle, fontweight='bold') 
        for i, ax in enumerate(axs.flat):
            if i < nMCFs:
                plotedMCF = False
                for c, case in enumerate(cases):
                    if 'MCF_BW_ref' in optimaltrajectories[case] and not plotedMCF:
                        plotedMCF = True
                        ax.plot(optimaltrajectories[case]['timeWithoutBuffers'][0,:-1].T,
                                optimaltrajectories[case]['MCF_BW_ref'][i:i+1,:].T, c='black', label='Mocap ' + cases[c], linewidth=linewidth)
                    if 'MCF_BW' in optimaltrajectories[case]:
                        ax.plot(optimaltrajectories[case]['time'][0,:-1].T,
                                optimaltrajectories[case]['MCF_BW'][i:i+1,:].T, c=colors[c], label='Dynamic simulation: ' + cases[c], linewidth=linewidth)         
                ax.set_title(MCF_labels[i], fontsize=fontsizeTitle, fontweight='bold')
                # ax.set_ylim((0,1))
                handles, labels = ax.get_legend_handles_labels()
        fig.align_ylabels()
        
        # Remove empty subplots.
        # for i in range(nMCFs, int(ny)**2):
        #     fig.delaxes(axs.flatten()[i])
        # Remove top and right spines.
        for ax in axs.flat:
            ax.spines['top'].set_visible(False)
            ...
            ax.tick_params(axis='both', which='major', labelsize=fontsizeTicks)
        # Add labels
        mm = int(ny)*(int(ny)-1) -(int(ny)*int(ny)-nMCFs)
        for i in range(mm,nMCFs):
            axs.flatten()[i].set_xlabel('Time (s)', fontsize=fontsizeLabel, fontweight='bold')
        # for ax in axs[:, 0]:
        #     ax.set_ylabel('()', fontsize=fontsizeLabel, fontweight='bold')
        # Add legend.
