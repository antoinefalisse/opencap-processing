'''
    ---------------------------------------------------------------------------
    OpenCap processing: extract_features.py
    ---------------------------------------------------------------------------
...
from data_info import get_data_case
...
for trial in trials_info:
    ...
    for leg in legs:
        ...
        gait_kinematics = gait_analysis(sessionDir, trialName_aligned, leg=leg,
                     lowpass_cutoff_frequency_for_coordinate_values=filter_frequency,
                     n_gait_cycles=-1, gait_style='overground')
        ...
        # %% Get kinematic features even if kinetics wasn't run
        kinematic_features_all = gait_kinematics.compute_scalars(scalar_names, return_all=True)
        kinematic_features = {}
        for scalar in scalar_names:
            kinematic_features
            kinematic_features[scalar] = {'value': kinematic_features_all[scalar]['value'].flatten()[gaitCycle],
                                          'units': kinematic_features_all[scalar]['units']}
            
        temp_events = gait_kinematics.get_gait_events()
            
        # Check that these are the same as the ones in the json file
        if kinetics_exist:
            for scalar in scalar_names:
                if scalar in gaitResults['scalars'].keys():
                    c_diff = np.abs(gaitResults['scalars'][scalar]['value'] - kinematic_features[scalar]['value'])
                    if c_diff > 1e-6:
                        raise ValueError('Scalar {} does not match between kinetics and kinematics, error: {}.'.format(scalar, c_diff))
